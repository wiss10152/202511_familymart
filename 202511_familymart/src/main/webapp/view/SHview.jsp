...
if (itemname != null && itemname.size() > 0) {
	...
	+ "<td><font color='white'>商品名</font></td>" + "<td><font color='white'>販売会社</font></td>"
	+ "<td><font color='white'>価格</font></td></tr>";

	for (int i = start; i < end; i++) {
		Itemfam item = itemname.get(i);

		Itemtable += "<tr><td>" + "<img src='" + request.getContextPath() + item.img
		+ "' width='120' height='120'></td>" + "<td><a href='GIcontrol?item=" + item.uriItemName
		+ "' target='_self'>" + item.ItemName + "</a></td>" + "<td>" + item.maker + "</td>" + "<td>"
		+ String.format("%,3d", item.price) + "円</td></tr>";
	}
